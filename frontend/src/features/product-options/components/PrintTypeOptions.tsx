        ...
        {PRINT_TYPE_OPTIONS.map((opt) => {
          ...
          return (
            <button
              ...
              className={`flex w-full items-start justify-between rounded-[8px] border p-2.5 text-left transition-all ${
                isSelected
            ...
            >
              <div className="flex min-w-0 gap-2">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-[6px] bg-[#FBFAF7] text-[#625F58]"><Printer className="h-3.5 w-3.5" /></span>
                <div>
                ...
                <p className="mt-0.5 text-[10px] leading-[15px] text-[#8A867E]">{opt.sublabel}</p>
              </div>
              </div>
              <div className="ml-3 flex shrink-0 items-center gap-2">
                <span className="text-[12px] font-medium text-[#171717]">
                  {(opt.priceDelta || 0) < 0 ? `-$${Math.abs(opt.priceDelta || 0)}` : 'Standard'}
                </span>
